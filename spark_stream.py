...
def insert_data(session, **kwargs):
    ...
    try:
        session.execute("""
            INSERT INTO spark_streams.created_users(id, first_name, last_name, gender, address, post_code, email,username, 
            dob, registered_date, phone, picture)
            VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
        """, (user_id, first_name, last_name, gender, address, post_code, email,username,
            dob, registered_date, phone, picture))
...
def create_selection_df_from_kafka(spark_df):
    ...
    sel = spark_df.selectExpr('CAST(value as string)').select(from_json(col("value"),schema).alias('data'))\
           .select("data.*")
    print(sel)
